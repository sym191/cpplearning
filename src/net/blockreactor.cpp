...
int net::blockreactor::foo(const unsigned short& domain, const unsigned short& protocol_type, const unsigned short& port,const char* ip){
    int sc = socket(domain, protocol_type, 0);
    struct sockaddr_in ad{domain, htons(port), inet_addr(ip)};
